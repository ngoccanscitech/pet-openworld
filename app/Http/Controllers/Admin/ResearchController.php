...
{
    public $data = [];
    public $admin_path = 'admin.research';
    public $title_head;
    /**
    ...
    public function index(){
        ...
        $dataReponse = [
            ...
            'url_create'  => route('admin.research_create'),
        ];
    ...
    public function create(){
        $dataReponse = [
            ...
            'url_action'  => route('admin.research_post'),
        ];
    ...
    public function post(Request $rq){
        ...
        if($post_id == 0){
            $respons = Research::create($data_db);
            $post_id= $respons->id;
        } else{
            $respons = Research::find($post_id)->update($data_db);
        }

        if(isset($data['category'])){
            \App\Model\ResearchCategoryJoin::where('research_id', $post_id)->delete();
            foreach ($data['category'] as $key => $category) {
                \App\Model\ResearchCategoryJoin::create([
                    'research_id'   => $post_id,
        ...
        if($save=='apply'){
            ...
            $url = route('admin.research_edit', array($post_id));
            Helpers::msg_move_page($msg, $url);
        }
        else{
            return redirect(route('admin.research'));
        }
    }
}
