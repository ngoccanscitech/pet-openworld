...
class CustomerController extends Controller
{
    ...
    public function author()
    {
        ...
        $this->data['users'] = \App\User::where('status', 1)->where('expert', 1)->paginate(20);
        $this->data['seo'] = [
            'seo_title' => 'Tác giả',
    ...
    {
        $user = User::where('id', $id)->where('expert', 1)->first();
        if($user)
        {
            $this->data['user'] = $user;
            $this->data['posts'] = \App\Model\Document::where('user_id', $user->id)->where('status', 1)->orderbyDesc('id')->paginate(20);
            ...
            return view($this->templatePath .'.author.detail', $this->data);
        }
    }
    ...
    public function updateAuthor()
    {
        ...
        $dataUpdate = [
            'expert'    => $data['author']??1,
            'job'    => $data['job']??'',
            'edu'    => $data['edu']??'',
            'type'    => $type,
        ];

        $user = User::where('id', $data['id'])->update($dataUpdate);
        return response()->json([
            'error' => 0,
            'message'   => 'success'
        ]);
